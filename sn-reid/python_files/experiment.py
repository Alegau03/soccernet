...
def ensemble_borda_count(dist_mats):
    """
    Rank-level fusion using Borda Count method.
    
    Instead of combining raw distances (which might be on different scales),
    this method assigns points based on rank position. The lowest distance
    gets rank 1. Borda count typically gives N points for 1st place, N-1 for 2nd, etc.
    Equivalently, we can just sum the ranks (lower sum = better final rank).
    
    Args:
        dist_mats (list): List of distance matrices from each model
        
    Returns:
        ndarray: A new 'distance' matrix representing the summed ranks.
    """
    n_queries, n_gallery = dist_mats[0].shape
    final_rank_sum = np.zeros((n_queries, n_gallery))
    
    for dist in dist_mats:
        # argsort twice gives the rank of each element (0-indexed)
        # np.argsort() gives the indices that would sort the array.
        # np.argsort() of that gives the rank of the original elements.
        ranks = np.argsort(np.argsort(dist, axis=1), axis=1)
        final_rank_sum += ranks
        
    return final_rank_sum
...
def calculate_biometric_metrics(dist_mat, labels_query, labels_gallery):
    """
    Calculate Biometric System metrics: System Response Reliability (SRR) and Equal Error Rate (EER).
    
    Args:
        dist_mat (ndarray): Distance matrix (M queries x N gallery)
        labels_query (ndarray): Identity labels for queries
        labels_gallery (ndarray): Identity labels for gallery
        
    Returns:
        tuple: (avg_srr, eer, fpr, fnr)
    """
    # 1. Calculate SRR (System Response Reliability)
    # Get the two smallest distances for each query
    sorted_dists = np.sort(dist_mat, axis=1)
    d1 = sorted_dists[:, 0]
    d2 = sorted_dists[:, 1]
    
    # max_d calculation can sometimes hit exactly 0 if matrix is all 0s, protect against division by zero
    max_d = np.max(dist_mat, axis=1)
    max_d[max_d == 0] = 1e-8 
    
    srr_scores = (d2 - d1) / max_d # Higher is more reliable
    avg_srr = np.mean(srr_scores)
    
    # 2. Preparation for DET/ROC (Match vs Non-Match scores)
    matches = []
    non_matches = []
    
    # Optimize finding matches using boolean masking for speed instead of nested loops
    for i in range(len(labels_query)):
        q_label = labels_query[i]
        match_mask = (labels_gallery == q_label)
        
        matches.extend(dist_mat[i, match_mask].tolist())
        non_matches.extend(dist_mat[i, ~match_mask].tolist())
     # 3. Calculate EER (Equal Error Rate) and DIR (Detect and Identification Rate)
    # Using distances as negative scores for ROC
    y_true = [1]*len(matches) + [0]*len(non_matches)
    y_scores = [-x for x in (matches + non_matches)]
    
    fpr, tpr, thresholds = roc_curve(y_true, y_scores)
    fnr = 1 - tpr
    
    # EER is point where FAR (fpr) == FRR (fnr)
    eer_idx = np.nanargmin(np.absolute((fpr - fnr)))
    eer = fpr[eer_idx]
    
    # Threshold at EER (negative of distance)
    # Match if score >= thresholds[eer_idx] -> distance <= -thresholds[eer_idx]
    eer_threshold = -thresholds[eer_idx]
    
    # DIR(t, 1): Probability that correct match is at rank 1 AND distance <= threshold
    # We already have rank-1 correct info for each query from the outer loop
    # Let's compute it properly.
    
    correct_at_rank1_and_below_threshold = 0
    num_queries = len(dist_mat)
    
    for i in range(num_queries):
        # Find index of candidate with smallest distance
        best_idx = np.argmin(dist_mat[i])
        best_dist = dist_mat[i, best_idx]
        
        # Check if it's a match (same PID)
        id_correct = (labels_query[i] == labels_gallery[best_idx])
        
        if id_correct and best_dist <= eer_threshold:
            correct_at_rank1_and_below_threshold += 1
            
    dir_eer = correct_at_rank1_and_below_threshold / num_queries
    
    # Calculate Margin M(t) at EER (should be ~0 by definition, but let's log it for completeness)
    margin_eer = np.abs(fpr[eer_idx] - fnr[eer_idx])
    
    return avg_srr, eer, fpr, fnr, dir_eer, margin_eer
...
class ExperimentLogger:
    ...
    def add_result(self, method, mAP, rank1, category="", srr=None, eer=None, dir_eer=None):
        """
        ...
        Args:
            ...
            srr (float, optional): Avg System Response Reliability
            eer (float, optional): Equal Error Rate
            dir_eer (float, optional): DIR at Rank-1 and EER threshold
        """
        self.results.append({
            'Category': category,
            'Method': method,
...
def run_experiments(args):
    ...
    log_dir = os.path.dirname(args.models[0])
    ...
    dataset = Soccernetv3(root='datasets', soccernetv3_training_subset=1.0, splits=["valid"])
    ...
    def evaluate(dist, name, category):
        """Compute metrics, including biometric metrics, and log results."""
        ...
        # Biometric metrics (SRR, EER, DIR, Margin)
        # Needs labels to compute False Rejection / False Acceptance
        avg_srr, eer, fpr, fnr, dir_eer, margin_eer = calculate_biometric_metrics(dist, pids_q, pids_g)
        
        logger.add_result(name, mAP, cmc[0], category, avg_srr, eer, dir_eer)
        
        if getattr(args, 'save_dist', False):
            save_name = f"dist_matrix_{arch}.npy"
            np.save(save_name, d)
            np.save("pids_q.npy", pids_q)
            np.save("pids_g.npy", pids_g)
            print(f"  [INFO] Saved distance matrix and PIDs: {save_name}, pids_q.npy, pids_g.npy")

        return mAP, cmc[0], avg_srr, eer, dist, dir_eer
    ...
    for i, (qf, gf) in enumerate(zip(q_feats_collection, g_feats_collection)):
        # Compute distance matrix for this model (needed for ensemble)
        d = compute_dist_matrix(qf, gf)
        dist_mats_single.append(d)
        
        # Evaluate and log (skip if requested)
        if not getattr(args, 'skip_single', False):
            mAP, r1, srr, eer, _, d_dir = evaluate(d, args.archs[i], "Single Model")
            logger.log(f"  {args.archs[i]:<35} | mAP: {mAP:>6.2%} | Rank-1: {r1:>6.2%} | SRR: {srr:.4f} | EER: {eer:>6.2%} | DIR: {d_dir:>6.2%}")
        else:
            logger.log(f"  {args.archs[i]:<35} | Skipping evaluation (distance matrix computed)")
    ...
    mAP, r1, srr, eer, _, d_dir = evaluate(d_concat, "Feature Concatenation", "Ensemble")
    logger.log(f"      {'Feature Concatenation':<31} | mAP: {mAP:>6.2%} | Rank-1: {r1:>6.2%} | SRR: {srr:.4f} | EER: {eer:>6.2%} | DIR: {d_dir:>6.2%}")
    
    # Method B: Distance Averaging (Equal Weights)
    logger.log("\n  [B] Distance Averaging (equal weights)")
    d_avg = ensemble_dist_avg(dist_mats_single)
    mAP, r1, srr, eer, _, d_dir = evaluate(d_avg, "Distance Avg (Equal)", "Ensemble")
    logger.log(f"      {'Distance Avg (Equal)':<31} | mAP: {mAP:>6.2%} | Rank-1: {r1:>6.2%} | SRR: {srr:.4f} | EER: {eer:>6.2%} | DIR: {d_dir:>6.2%}")
    
    # Method C: Rank-Level Fusion (Borda Count)
    logger.log("\n  [C] Rank-level Fusion (Borda Count)")
    if len(dist_mats_single) > 1:
        d_borda = ensemble_borda_count(dist_mats_single)
        mAP, r1, srr, eer, _, d_dir = evaluate(d_borda, "Borda Count (Rank Fusion)", "Ensemble")
        logger.log(f"      {'Borda Count':<31} | mAP: {mAP:>6.2%} | Rank-1: {r1:>6.2%} | SRR: {srr:.4f} | EER: {eer:>6.2%} | DIR: {d_dir:>6.2%}")
    
    # Method D: Weighted Distance Fusion - Grid Search
    logger.log("\n  [D] Weighted Distance Fusion (grid search)")
    ...
    if len(dist_mats_single) == 2:
        ...
        for w1 in [0.3, 0.4, 0.5, 0.6, 0.7]:
            ...
            mAP, r1, srr, eer, _, d_dir = evaluate(d_w, f"Weighted {w1:.1f}-{w2:.1f}", "Ensemble Grid Search")
            logger.log(f"      w=({w1:.1f}, {w2:.1f}) -> mAP: {mAP:>6.2%} | EER: {eer:>6.2%} | DIR: {d_dir:>6.2%}")
            if mAP > best_w_map:
                best_w_map, best_w_config = mAP, f"({w1:.1f}, {w2:.1f})"
                best_dist_w, best_r1 = d_w, r1
        ...
        for w1, w2, w3 in weight_configs:
            ...
            mAP, r1, srr, eer, _, d_dir = evaluate(d_w, f"Weighted {w1:.2f}-{w2:.2f}-{w3:.2f}", "Ensemble Grid Search")
            logger.log(f"      w=({w1:.2f}, {w2:.2f}, {w3:.2f}) -> mAP: {mAP:>6.2%} | EER: {eer:>6.2%} | DIR: {d_dir:>6.2%}")
            if mAP > best_w_map:
    ...
    for cfg in rr_configs:
        ...
        mAP, r1, srr, eer, _, d_dir = evaluate(d_rr, f"Re-Rank: {cfg['name']}", "Re-Ranking")
        logger.log(f"      mAP: {mAP:>6.2%} | Rank-1: {r1:>6.2%} | SRR: {srr:.4f} | EER: {eer:>6.2%} | DIR: {d_dir:>6.2%}")

    # ==========================================================================
    # GOAT ANALYSIS (Qualitative Biometrics)
    # ==========================================================================
    if target_dist_for_rr is not None:
        logger.log_section("GOAT ANALYSIS (Qualitative Biometrics)")
        logger.log("  Identifying the top 5 'Goats' (lowest SRR queries) from the best ensemble...")
        
        # Calculate SRR for the best distance matrix to find Goats
        d_best = target_dist_for_rr
        sorted_dists = np.sort(d_best, axis=1)
        d1 = sorted_dists[:, 0]
        d2 = sorted_dists[:, 1]
        max_d = np.max(d_best, axis=1)
        max_d[max_d == 0] = 1e-8 
        srr_scores = (d2 - d1) / max_d
        
        # Get indices of the 5 lowest SRR scores
        goat_indices = np.argsort(srr_scores)[:5]
        
        out_dir = "figures/goats"
        os.makedirs(out_dir, exist_ok=True)
        logger.log(f"  Saving Goat visualizations to {out_dir}/...")
        
        for rank, q_idx in enumerate(goat_indices):
            srr_val = srr_scores[q_idx]
            q_img_path = dataset.query[q_idx][0]
            q_pid = pids_q[q_idx]
            
            # Get top 10 gallery indices
            top10_g_indices = np.argsort(d_best[q_idx])[:10]
            
            # Create a combined image like Gradio
            try:
                img_width, img_height = 128, 256
                margin = 5
                
                # Canvas size: 1 query + 10 gallery
                canvas_w = (img_width + margin) * 11 + margin
                canvas_h = img_height + 2 * margin + 20 # 20 for text
                canvas = Image.new('RGB', (canvas_w, canvas_h), (255, 255, 255))
                draw = ImageDraw.Draw(canvas)
                
                # Paste query
                q_img = Image.open(q_img_path).convert('RGB').resize((img_width, img_height))
                q_with_border = Image.new('RGB', (img_width + 8, img_height + 8), 'blue')
                q_with_border.paste(q_img, (4, 4))
                canvas.paste(q_with_border.resize((img_width, img_height)), (margin, margin + 20))
                draw.text((margin, margin), f"Q: ID {q_pid} (SRR: {srr_val:.4f})", fill='black')
                
                for j, g_idx in enumerate(top10_g_indices):
                    g_img_path = dataset.gallery[g_idx][0]
                    g_pid = pids_g[g_idx]
                    is_correct = (q_pid == g_pid)
                    border_color = 'green' if is_correct else 'red'
                    
                    g_img = Image.open(g_img_path).convert('RGB').resize((img_width, img_height))
                    g_with_border = Image.new('RGB', (img_width + 10, img_height + 10), border_color)
                    g_with_border.paste(g_img, (5, 5))
                    
                    x_offset = margin + (img_width + margin) * (j + 1)
                    canvas.paste(g_with_border.resize((img_width, img_height)), (x_offset, margin + 20))
                    
                save_path = os.path.join(out_dir, f"goat_rank{rank+1}_srr{srr_val:.4f}.png")
                canvas.save(save_path)
                logger.log(f"    Saved Rank {rank+1} Goat (SRR: {srr_val:.4f}) -> {save_path}", also_print=False)
            except Exception as e:
                logger.log(f"    Failed to save Goat {rank+1}: {e}")
    ...
    header = f"  {'Method':<43} | {'mAP':>8} | {'Rank-1':>8} | {'SRR':>8} | {'EER':>8} | {'DIR':>8}"
    logger.log(header)
    logger.log("  " + "-" * 105)
    ...
    for cat in categories:
        ...
        if cat_results:
            ...
            for r in cat_results:
                srr_str = f"{r['SRR']:.4f}" if r.get('SRR') is not None else "N/A"
                eer_str = f"{r['EER']:>7.2%}" if r.get('EER') is not None else "N/A"
                dir_str = f"{r['DIR']:>7.2%}" if r.get('DIR') is not None else "N/A"
                logger.log(
                    f"    {r['Method']:<41} | {r['mAP']:>7.2%} | {r['Rank-1']:>7.2%} | {srr_str:>8} | {eer_str:>8} | {dir_str:>7}"
                )
            logger.log("")
...
Examples:
  ...
  python experiment.py \\
    ...
    parser.add_argument('--skip-single', action='store_true', help='Skip Experiment 1 (single models)')
    parser.add_argument('--save-dist', action='store_true', help='Save distance matrices and labels for standalone analysis')
    parser.add_argument('--re-rank', action='store_true', help='Apply re-ranking (Standard)')
