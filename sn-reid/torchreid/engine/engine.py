...
from torchreid.utils import (
    ...
    open_specified_layers, visualize_ranked_results, load_checkpoint
)
...
class Engine(object):
    ...
    def run(
        ...
        if self.start_epoch > 0:
            ...
            # Check for model-best.pth.tar in subdirectories corresponding to model names
            names = self.get_model_names()
            found_best = False
            for name in names:
                # The default save logic puts it in save_dir/name/model-best.pth.tar
                best_model_path = osp.join(save_dir, name, 'model-best.pth.tar')
                if not osp.exists(best_model_path):
                     # Fallback 1: Check if save_dir IS the specific folder (sometimes user passes child dir)
                     best_model_path = osp.join(save_dir, 'model-best.pth.tar')
                
                if osp.exists(best_model_path):
                    print('=> Attempting to load best_rank1 from "{}"'.format(best_model_path))
                    try:
                        # Load to CPU to avoid consuming GPU memory needed for training
                        checkpoint = torch.load(best_model_path, map_location='cpu')
                        if 'rank1' in checkpoint:
                            self.best_rank1 = checkpoint['rank1']
                            print('=> Loaded best_rank1 = {:.1%}'.format(self.best_rank1))
                            found_best = True
                        
                        # Explicitly delete checkpoint and force garbage collection to free memory
                        del checkpoint
                        import gc
                        gc.collect()
                        ...
                        print('=> Failed to load best_rank1 from "{}": {}'.format(best_model_path, e))
            
            if not found_best:
                 print('=> No previous "model-best.pth.tar" found to restore best_rank1. Starting with -inf.')
            ...
            if (self.epoch + 1) >= start_eval \
               ...
               and (self.epoch + 1) != self.max_epoch:
                ...
                if is_best:
                    ...
                    print("=> New best Rank-1 {:.1%}".format(rank1))
