...
def eval_soccernetv3(distmat, q_pids, g_pids, q_action_indices, g_action_indices, max_rank):
    ...
    for q_idx in range(num_q):
        ...
        # compute cmc curve
        # Optimizing memory: compare only relevant rows
    ...
    for cmc in all_cmc:
        if cmc.size < max_rank:
            # Pad with the last value (cumulative)
            padding = np.ones(max_rank - cmc.size, dtype=np.int64) * cmc[-1]
            new_cmc = np.concatenate((cmc, padding))
        else:
            new_cmc = cmc[:max_rank]
        new_all_cmc.append(new_cmc)

    all_cmc = np.asarray(new_all_cmc).astype(np.float32)
